...
async def user_file_handler(event: NewMessage.Event | Message):
    ...
    dl_link = f'{Server.BASE_URL}/dl/{message_id}?link={secret_code}'
    tg_link = f'{Server.BASE_URL}/file/{message_id}?link={secret_code}'
    ...
    if (event.document and 'video' in event.document.mime_type) or event.video:
        stream_link = f'{Server.BASE_URL}/stream/{message_id}?link={secret_code}'
        await event.reply(
